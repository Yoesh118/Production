...
import org.production.portal.web.validator.production.IncidentCostValidator;
 ...
 */
@Controller
@RequestMapping("/productionCost/incidentCost")
public class IncidentCostController extends BaseController {

    @Resource
    private IncidentCostService incidentCostService;
    @Resource
    private IncidentCostValidator incidentCostValidator;
    @Resource
    ...
    private String setUpModel(ModelMap model, IncidentCost item) {
        ...
        model.addAttribute("workIncident", workIncidentService.getAll());
        ...
        return "product/incidentCostForm";
    }
    ...
    public String getItemForm(ModelMap model, @RequestParam(required = false) String id, @RequestParam(required = false) String productionCostId) {
        IncidentCost p;
        if (id != null) {
            p = incidentCostService.get(id);
            return setUpModel(model, p);
        }
        p = new IncidentCost(productionCostService.get(productionCostId));
        return setUpModel(model, p);
    }

    @RequestMapping(value = "/item.form", method = RequestMethod.POST)
    public String saveItem(@ModelAttribute("item") @Valid IncidentCost incidentCost, BindingResult result, ModelMap model) {
        incidentCostValidator.validate(incidentCost, result);
        if (result.hasErrors()) {
            model.addAttribute("message", new AppMessage.MessageBuilder(Boolean.TRUE).message("Data entry error has occurred").messageType(MessageType.ERROR).build());
            setUpModel(model, incidentCost);
            return "product/incidentCostForm";
        }
        incidentCostService.save(incidentCost);
        return "redirect:../dashboard/profile.htm?type=1&id=" + incidentCost.getProductionCost().getProductionCostId();
    }

    @RequestMapping(value = "item.delete", method = RequestMethod.GET)
    public String getDeleteForm(@RequestParam("id") String id, ModelMap model) {
        IncidentCost incidentCost = incidentCostService.get(id);
        ItemDeleteDTO dto = new ItemDeleteDTO(id, incidentCost.getProductionCost().getName(), "../dashboard/profile.htm?type=3&id="+incidentCost.getProductionCost().getProductionCostId());
        model.addAttribute("item", dto);
        model.addAttribute("message", new AppMessage.MessageBuilder(Boolean.TRUE).message("Are you sure you want to delete this record").messageType(MessageType.WARNING).build());
        model.addAttribute("pageTitle", appPrefix+"Delete " + incidentCost.getProductionCost().getName());
        return "admin/deleteItem";
    }

    @RequestMapping(value = "item.delete", method = RequestMethod.POST)
    public String deleteItem(@Valid ItemDeleteDTO dto) {
        IncidentCost contact = incidentCostService.get(dto.getId());
        ProductionCost p = contact.getProductionCost();
        incidentCostService.delete(contact);
